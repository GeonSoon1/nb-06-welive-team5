...
if (process.env.NODE_ENV !== 'test') {
  ...
  // 수동 AWS 키는 "둘 다 있거나 둘 다 없어야" 한다. 없으면 EC2 IAM Role 경로를 사용한다.
  if (hasAwsAccessKey !== hasAwsSecretKey) {
    throw new Error("[Config Error] AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY는 둘 다 설정하거나 둘 다 비워야 합니다.");
  }
}
...
export const AWS_ACCESS_KEY_ID = process.env.AWS_ACCESS_KEY_ID?.trim() || undefined;
export const AWS_SECRET_ACCESS_KEY = process.env.AWS_SECRET_ACCESS_KEY?.trim() || undefined;
