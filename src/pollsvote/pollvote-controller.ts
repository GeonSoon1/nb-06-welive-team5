...
import { pollsVoteService } from './pollvote-service.js';
import { RequestWithUser } from './pollvote-struct';

class PollsVoteController {
    addVote: ExpressHandler = async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
        try {
    ...
    cancelVote: ExpressHandler = async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
        try {
            const { optionId } = req.params;
            const userId = (req as RequestWithUser).user?.userId;
            if (!userId) {
...
export const pollsVoteController = new PollsVoteController();
